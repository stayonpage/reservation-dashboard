-- 수동 예약 입력에 전화번호 입력란 추가 — 네이버·아임웹은 원래 메일에 번호가 없지만,
-- 수동 입력은 직원이 따로 알고 있는 번호를 채울 수 있게 허용(공란이면 null 그대로).
...
begin
  ...
  insert into reservations (
    ...
    p_channel, v_synthetic_id, p_guest_name, p_guest_phone, p_room_name,
...
grant execute on function create_manual_reservation(
  channel, text, text, date, date, integer, payment_status, jsonb, text
) to authenticated;
